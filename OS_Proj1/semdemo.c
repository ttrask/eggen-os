...
printf("%5d %d %s\n",getpid(),semctl(semid,0,GETVAL,arg.sem_num),"ONE");

    printf("Press return to block: ");
    getchar();
    printf("Trying to block...\n");
printf("%5d %d %s\n",getpid(),semctl(semid,0,GETVAL,arg.sem_num),"TWO");
/* decrements to 0 */
    ...
    printf("%5d    %d     %s\n\n",
        getpid(), semctl(semid, 0, GETVAL, arg.sem_num), "THREE");
...
printf(" tries to decrement to -1 but hangs since the semaphore is 0\n");
    ...
    printf("%5d    %d     %s\n\n",
        getpid(), semctl(semid, 0, GETVAL, arg.sem_num), "FOUR");
...
/*    sb.sem_op = 1; */ /* free resource */
    ...
    printf("Unblocked\n");
    printf("%5d    %d     %s\n",
       getpid(), semctl(semid, 0, GETVAL, arg.sem_num), "incr");
